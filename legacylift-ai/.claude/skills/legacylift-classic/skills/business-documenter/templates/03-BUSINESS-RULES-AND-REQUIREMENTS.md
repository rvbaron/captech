...
<!-- DOMAIN ORDERING: Do NOT list domains in arbitrary or alphabetical order.
     Order by business process flow and data dependency:
     Layer 1: Auth/Identity (entry point)
     Layer 2: Reference data (catalog, users, products — queried by other domains)
     Layer 3: Pricing/valuation (depends on reference data)
     Layer 4: Core business logic (suggestions, opportunities — depends on pricing + reference data)
     Layer 5: Consumer-facing output (recommendations, fulfillment — depends on core logic)
     Layer 6: Supporting/cross-cutting (translations, auditing, compliance)
